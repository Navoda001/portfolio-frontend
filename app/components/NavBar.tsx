        ...
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <div className="hidden lg:flex items-center space-x-8">
            {navItems.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className={`relative px-3 py-2 text-sm font-medium tracking-wide transition-all duration-300 group ${isActive(item.href)
                    ? 'text-emerald-400'
              ...
              >
                {item.name}
                <span
                  className={`absolute bottom-0 left-0 h-0.5 bg-emerald-400 transition-all duration-300 ${isActive(item.href)
                      ? 'w-full'
          ...
          <div className="hidden lg:block">
            <Link
              ...
              className="relative px-6 py-2.5 bg-emerald-500 text-white text-sm font-semibold rounded-full overflow-hidden group transition-all duration-300 transform hover:-translate-y-1 hover:shadow-lg hover:shadow-emerald-500/25"
            >
          ...
          >
            <span
              className={`w-6 h-0.5 bg-white transition-all duration-300 origin-center ${isMobileMenuOpen ? 'rotate-45 translate-y-2' : ''
                }`}
            />
            <span
          ...
          <div className="px-2 pt-2 pb-6 space-y-1 bg-gray-900/95 backdrop-blur-md rounded-lg mt-2 border border-gray-800/50">
            {navItems.map((item) => (
              <Link
                ...
                className={`block px-4 py-3 text-base font-medium rounded-lg transition-all duration-300 ${isActive(item.href)
